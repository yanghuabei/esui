...
define(
    function (require) {
        ...
        function getAddonHTML() {
            var searchIconTPL = '<span class="${searchIconClasses}"></span>';
            // button内容默认为图标
            var searchButtonTPL = ''
                + '<esui-button data-ui="childName:search;variants:${buttonVariants}" class="${searchClasses}">'
                +     (this.buttonContent ? this.buttonContent : searchIconTPL)
                + '</esui-button>';
            var addonContent = searchButtonTPL;
            // 即时搜索不需要搜索按钮
            if (this.searchMode === 'instant') {
                addonContent = searchIconTPL;
            }
            var tpl = ''
